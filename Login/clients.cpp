...
query.prepare("UPDATE clients SET NOM=:nom,PRENOM=:prenom,ADRESSE=:adresse WHERE ID=:id");
query.bindValue(":id", res);
query.bindValue(":nom", nom);
query.bindValue(":prenom", prenom);
query.bindValue(":adresse", adresse);
...
model->setQuery("select * from clients order by nom");// ORDER CRIER EN FONCTION DE CE QUE TU VEUX
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("Nom "));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("Prénom"));
model->setHeaderData(3, Qt::Horizontal, QObject::tr("Adresse"));
