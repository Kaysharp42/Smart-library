...
timer->start(1000); //argument: time il ms
}
...
{
    QPrinter printer;
    printer.setPrinterName("Hp_imprimante"); //selon le nom de mon imprimante
    QPrintDialog dialog(&printer, this);
    //ceck printer correct or not

    if (dialog.exec() == QDialog::Rejected) return; //on ne continue plus
...
void MainWindow::myfunction()
{
    ...
    QString time_string = time.toString("hh : mm : ss");//convertir time to string
    ui->label_34->setText(time_string);
}
